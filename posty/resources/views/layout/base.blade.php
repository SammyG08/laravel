            ...
            <div class="d-flex p-2 flex-row bg-white bg-gradient justify-content-between">
                <div class="btn-group bg-transparent bg-gradient ">
                    <button class="btn btn-md btn-outline-light ">
                        <a href="{{route('home')}}" class=" text-decoration-none fs-6 p-1 text-secondary {{request()->routeIs('home') ? 'fw-bold text-dark' : ''}}">Home</a>
                    </button>
                    ...
                    <button class="btn btn-outline-light btn-md">
                        <a href="{{route('post')}}" class="fs-6 text-secondary text-decoration-none p-1 {{request()->routeIs('post') ? 'fw-bold text-dark' : ''}}">Posts</a>
                    </button>
                </div>
                <div class="btn-group bg-transparent bg-gradient">
                    @auth
                    <button class=" text-secondary btn btn-md btn-outline-light fs-6 pe-3 ">{{auth()->user()->name}}</button>
                    <form action="{{route('logout')}}" method="post" class="fs-6 ">
                        @csrf
                        <button type="submit" class="text-secondary bg-transparent btn btn-outline-light p-2">Logout</button>
                    </form>
                    @endauth
                    @guest
                    <button class="btn btn-outline-light fs-6 btn-md">
                        <a href="{{route('login')}}" class=" text-secondary text-decoration-none p-1 {{request()->routeIs('login') ? 'fw-bold text-dark' : ''}}">Login</a>
                    </button>
                    <button class="btn btn-md btn-outline-light fs-6">
