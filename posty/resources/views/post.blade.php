...
<div class="container bg-transparent text-muted bg-gradient p-2 text-center fs-3 mb-4">Posts</div>
<div class=" w-50 container-fluid rounded-3 border-1 text-dark bg-white bg-gradient shadow-sm p-5">
    <div class="p-3">
        @auth
        <form action="{{ route('post')}}" method = "post" class="form-group mb-3 ">
            @csrf
            <textarea name="body" class="form-control bg-light text-dark mt-4 mb-3 @error('body') border-3 border-danger @enderror " placeholder="Post Something" rows="5"></textarea>
            @error('body')
