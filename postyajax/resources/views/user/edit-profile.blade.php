...
<div class="container bg-transparent w-50 mb-4 p-3">
    ...
    <div class="container-fluid mt-4 w-50">
        <form enctype="multipart/form-data" action="{{route('store.edit', $user)}}" class="form-group container mt-4 w-100 ps-0" method="POST">
            @csrf
            <input type="file" class="form-control w-100 mb-4 border border-0" name="image">
            <label for="name" class="form-label">Name:</label>
            <input type="text" name="name" placeholder="{{$user->name}}" class="form-control w-100 mb-0 border border-0">
            @error('name')
            <span class="text-danger d-block mb-4">{{$message}}</span>
            @enderror
            <label for="username" class="form-label">Userame:</label>
            <input type="text" name="username" placeholder="{{$user->username}}" class="form-control mb-0 w-100 border border-0">
            @error('username')
            <span class="text-danger d-block mb-4">{{$message}}</span>
            @enderror
