                    ...
                    <div class="mb-3 pb-3 container-fluid ps-4 pt-4 bg-light d-flex align-items-center">
                        ...
                        <div>
                            ...
                            @auth
                                <div class="mt-1">
                                    @if (!$comment->likedBy(auth()->user()))
                                        <div class="d-inline">
                                            <form action="{{ route('comment.like', [$post, $comment]) }}" method="post"
                                    ...
                                    @if ($comment->user_id === auth()->user()->id)
                                        <div class="d-inline">
                                            <form action="{{ route('delete.comment', [$post, $comment]) }}" method="post"
                                                class="w-100 d-inline">
                                                @csrf
                                                @method('DELETE')
                                                <button
                            ...
                            @if ($comment->replies->count())
                                <a href="{{ route('show.replies', $comment) }}" class=" d-block text-primary"><small>Show
                                        replies</small></a>
                            @endif
